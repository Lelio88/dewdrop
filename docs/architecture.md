...
| `lib/decor/decor_backdrop.dart` | **Fond parallaxe partagé** par tous les décors : auto-découvre les couches `assets/<photo\|illustrated>/<env>/<variant>/`, applique tilt + auto-pan, et supporte un **FX inter-couches** (`midFx`/`midFxBelow`) occulté par les plans avant (oiseaux entre les arbres, étoiles filantes derrière un pic). |
...
- **Pipeline unifié (photo & dessin)** : depuis `environment.dart`, **chaque décor** = un `DecorBackdrop` (couches parallaxe) **+ son painter FX bespoke par-dessus**, sur les **deux** modes. Le `RenderMode` choisit juste l'arbre d'assets — `assets/photo/` (photoréaliste) ou `assets/illustrated/` (aquarelle) : **même scène, mêmes FX**. Les anciens fonds procéduraux servent de **fallback** le temps que les couches chargent. (`photo_decor.dart` subsiste comme legacy, plus routé.)
- **Couches & bords nets** : `assets/<root>/<env>/<variant>/0.webp` = fond … `N.webp` = avant. Chaque couche a son fond **reconstruit par inpainting LaMa** (le plus proche est effacé puis comblé) pour éviter la **« trace »** du sujet quand un plan glisse. Profondeur via **Depth-Anything V2 Large** + **guided filter** (alpha *edge-aware* → supprime l'effet d'escalier) ; feather réduit par scène. Nb de plans + feather **par scène** (`SCENE_SETTINGS`), `depthStrength` par décor atténue le parallaxe là où la profondeur est peu fiable (espace ≈ plat). Pipeline : `split_all.py` (DA-V2-Large + LaMa, GPU si torch CUDA) → `tools/depth_split/_src/` → webp.
- **Mode dessin (aquarelle)** : `illustrate_all.py` stylise chaque source en **aquarelle storybook** (SDXL img2img, force par scène), **sans humain** (negative prompt) ni **filigrane** (`clean_watermark.py`) ; `split_illustrated.py` produit `assets/illustrated/`.
