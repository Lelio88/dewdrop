...
| `tour` | **Tuto d'accueil** en bulles-nuage (`cloud_tour.dart` + `cloud_bubble.dart`) : 5 étapes ancrées sur les vrais éléments (poignées haut/bas, ☰), scénario **pur** dans `domain/tour_step.dart`. Le voile **laisse passer les gestes** (il ne capte que le tap) : une étape portant un `TourGesture` se valide quand l'utilisateur fait *réellement* le glissement, signalé par `HomeView` via un `ValueNotifier`, puis avance après ~1,1 s. La bulle **glisse** d'une position à l'autre (`AnimatedPositioned` sur un `top` unique, hauteur mesurée post-frame). Vu/pas-vu en `SharedPreferences` (`home_tour_seen`) via `homeTourProvider` ; rejouable depuis Réglages. |
