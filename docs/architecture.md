...
| `tour` | **Tuto d'accueil** en bulles-nuage (`cloud_tour.dart` + `cloud_bubble.dart`) : 5 étapes ancrées sur les vrais éléments (poignées haut/bas, ☰), scénario **pur** dans `domain/tour_step.dart`. Vu/pas-vu en `SharedPreferences` (`home_tour_seen`) via `homeTourProvider` ; rejouable depuis Réglages. |
