...
| `tour` | **Tutos en bulles-nuage** (`cloud_tour.dart` + `cloud_bubble.dart`), scénarios **purs** dans `domain/tour_step.dart`. **Quatre tours, un par écran** (`TourId`) : l'accueil (8 étapes, les gestes) au premier lancement, puis **Amis / Univers / Réglages** qui affichent chacun 2 bulles à leur première ouverture — tout expliquer d'un coup ferait un mur que personne ne lit. **Pendant un tuto, le tuto possède l'écran** : le voile est `opaque` et n'offre que trois issues — faire le geste demandé, taper pour avancer, « Passer le tuto ». C'est ce qui permet à une bulle de *décrire* l'écran : laisser l'utilisateur le modifier en pleine phrase fait diverger les mots et les pixels, et toute tentative de resynchroniser après coup est un rustinage. Le glissement est reconnu **par le tuto**, qui le confie à `onPerform` (l'accueil l'exécute pour de vrai) puis avance après ~1,6 s ; **tout** geste non attendu est **refusé visiblement** (le nuage frissonne), y compris sur une étape qui n'en demande aucun — un refus silencieux se lit comme une app figée. Chaque bulle **annonce sa sortie** (« Glisse vers le haut… » / « Appuie n'importe où… ») : sans ça, une étape sans geste ressemble trait pour trait à une étape avec geste, et le lecteur glisse sur un écran qui ne répondra pas. Une étape porte aussi une `TourScene` : le tuto **met la scène en place** (tiroir entrouvert / plein écran) pour ne jamais décrire ce qui n'est pas à l'écran. La cible est **re-résolue à chaque frame** → la bulle *chevauche* le tiroir qui monte au lieu de le masquer. Quand la cible occupe presque tout l'écran (tiroir plein écran), « à côté de la cible » n'existe plus : l'étape déclare alors un `TourPlacement` (bord haut/bas) et vise une **partie** du panneau (la bande du chevron), sans quoi la bulle retombe sur le contenu qu'elle décrit. Les trois **bouffées** de la queue ne s'affichent que si elles relient vraiment quelque chose (≤ 90 px) : au-delà elles s'arrêtent dans le vide et se lisent comme des miettes collées au nuage — le halo suffit alors. **Coût de rendu** : la silhouette (9 unions de `Path`) est mémoïsée, bulle et voile ont chacun leur `RepaintBoundary` (déplacer l'un ne repeint plus l'autre — trois passes floutées à chaque frame sinon), et le `saveLayer` plein écran n'est payé que lorsqu'il y a un halo à découper. **Ancre et placement se décident ensemble** : emprunter l'un sans l'autre faisait bouger la bulle deux fois, la seconde sans animation. Vu/pas-vu par tour en `SharedPreferences` (`tour_seen_<id>`) ; Réglages les réarme tous. |
...
- ❌ Une **tuile Material dans une carte colorée** sans `Material` à elle. `ListTile` peint son fond et son encre sur le `Material` le plus proche : tout `ColoredBox`/`DecoratedBox` à `color` non transparente rencontré avant lui (nos cartes en verre, la feuille du menu) les recouvre, et le retour tactile disparaît. Flutter le signale depuis `ListTile._debugCheckBackgroundIsHidden`, mais **en debug seulement** — le défaut visuel, lui, est bien présent en release. Correctif : `Material(type: MaterialType.transparency)` autour du **groupe** de tuiles, à l'intérieur de la carte (le `Scaffold` fournit déjà `bodyMedium`, le rendu du texte ne bouge pas). Un `BoxDecoration` à `gradient` sans `color:` — `GlassCard`, les fonds d'écran — n'est pas concerné. Se vérifie en montant l'écran dans un `testWidgets` : le binding transforme le signalement en échec.
- ❌ Armer Crashlytics en **debug**. `FlutterError.onError = recordFlutterFatalError` promeut chaque diagnostic du framework — y compris ceux qui ne lèvent rien, comme celui ci-dessus — en crash **fatal** imputé à un appareil réel. Le tableau de bord se remplit d'incidents que personne n'a vécus, d'autant plus trompeurs que l'émulateur Android se déclare constructeur « Google ». `setCrashlyticsCollectionEnabled(!kDebugMode)` au démarrage ; `presentError` continue d'écrire en console.
