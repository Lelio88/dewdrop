...
- **Dernier focus** : **tiroir d'envoi relu** — les deux familles ne partagent plus une rangée : amis (ronds) puis cercles (carrés), chacun défilant à part au 1er cran et en sections libellées au 2e, le lien « Mes amis & cercles » remonté dans le header à droite. Et les **cercles se trient par récence** comme les amis, via une **seconde** lecture (`recentlyContactedGroupIds`, filtrée `group_id is not null`) : les fondre dans la fenêtre des amis aurait laissé un fan-out de 20 membres évincer tous les autres cercles. Avant : **pensées de groupe lisibles** — `group_id` remonte jusqu'à l'écran, le nom du groupe est résolu à l'affichage, la phrase (`thoughtLine`) est partagée app ↔ notif.
- **Focus immédiat** : la **disposition** du tiroir est constatée (captures rendues en 360, 320 et sans aucun cercle) ; le **tri des cercles par récence** ne l'est pas encore sur appareil — sa requête est correcte et couverte par la RLS et le GRANT existants, mais aucune migration ne la protège d'une régression, seul un envoi réel la prouvera. Reste aussi sans preuve le **bouton « ajouter en ami » dans un groupe**, jamais essayé sur un téléphone, et le branchement UI de la **suggestion de handle** (son versant serveur, lui, est vérifié par `verify_prod.py`).
