...
- **Focus immédiat** : vérifications **on-device** restantes — le serveur est à jour et vérifié (`verify_prod.py` vert : 6 gabarits FR servis, expéditeur SMTP, `search_profiles` et ses garde-fous, GRANTs `service_role` des tables lues par la fonction push). Sur un vrai téléphone : (a) le tuto au premier lancement + son rejeu depuis Réglages (spotlights alignés sur les poignées et le ☰) ; (b) la suggestion de handle après une faute de frappe, et le bouton « ajouter en ami » dans un groupe.
