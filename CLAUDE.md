...
- **Focus immédiat** : une seule chose reste sans preuve — le **bouton « ajouter en ami » dans un groupe**, jamais essayé sur un téléphone. La **suggestion de handle** (« Tu voulais dire… ») est tenue pour bonne de mémoire, pas constatée : son versant serveur est vérifié (`verify_prod.py` contrôle `search_profiles` et ses garde-fous 0.45 / limite 3), seul le branchement UI repose sur un souvenir. Tout le reste est vérifié — serveur par `verify_prod.py`, tuto et lisibilité des pensées de groupe sur appareil.
