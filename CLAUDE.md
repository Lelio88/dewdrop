...
- **Dernier focus** : **pensées de groupe lisibles** — une pensée de groupe se disait « X a pensé à toi », indistinguable d'une perso (retour utilisateur) : `group_id` remonte désormais jusqu'à l'écran, le nom du groupe est résolu à l'affichage, et la phrase (`thoughtLine`) est partagée app ↔ notif. Avant : lot **accueil & découvrabilité** — (1) **tuto d'accueil** en bulles-nuage (feature `tour`, 5 étapes ancrées, rejouable depuis Réglages) qui remplace le hint texte éclair ; (2) **tous** les gabarits d'email auth désormais surchargés en FR (`email_change`, `magic_link`, `reauthentication`, `invite` s'ajoutent aux deux existants) + test de non-régression ; (3) **demande d'ami entre co-membres d'un groupe** ; (4) renvoi vers « Amis » en pied de l'écran d'envoi ; (5) repêchage **« Tu voulais dire… »** après un handle introuvable (migration `20260817120000_profiles_fuzzy_handle_search`, RPC `search_profiles`).
- **Focus immédiat** : vérifications **on-device** uniquement — le serveur est à jour (`verify_prod.py` vert : les 6 gabarits FR servis, expéditeur SMTP, `search_profiles` et ses garde-fous ; Edge Function `send-thought-push` déployée). Sur un vrai téléphone : (a) une pensée de groupe nomme bien son cercle — « X a pensé au groupe « Y » » — dans la notif **et** dans la liste ; si elle dit « à un groupe » alors qu'on en est toujours membre, c'est la lecture du nom qui échoue côté serveur, bug distinct ; (b) le tuto au premier lancement + son rejeu depuis Réglages (spotlights alignés sur les poignées et le ☰) ; (c) la suggestion de handle après une faute de frappe, et le bouton « ajouter en ami » dans un groupe.
