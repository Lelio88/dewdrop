...
| Étape / texte du tuto d'accueil | `features/tour/domain/tour_step.dart` (`kHomeTour`, script pur) ; une nouvelle **cible** = valeur `TourAnchor` + `GlobalKey` posée sur le widget réel + entrée dans la map `anchors` (`home_screen.dart`) ; un nouveau **geste à faire** = valeur `TourGesture` + signalement depuis le handler de drag concerné (`_tourGesture.value = …`) |
